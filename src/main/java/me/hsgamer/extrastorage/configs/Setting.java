...
public final class Setting
    ...
    public void setup() {
        ...
        this.maxSpace = config.getLong("MaxSpace", 700000);
        ...
        this.blacklist = config.getStringList("Blacklist")
                ...
                .map(string -> {
                    ...
                    if (key.matches("(?ium)(I(tems)?A(dder)?|Oraxen)"))
                        return string;
                    return string;
                })
    ...
    public String getNameFormatted(Object key, boolean colorize) {
        ...
        if (!name.isEmpty())
            return (colorize ? name : Utils.stripColor(name));
    ...
    public Map<String, String> getMaterialTypes() {
