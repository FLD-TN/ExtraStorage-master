...
public class MaterialTypeConfig {
    ...
    /**
     * Kiểm tra xem vật liệu có được phép lưu trữ không dựa trên phương pháp
     * whitelist
     * 
     * @param material Loại vật liệu cần kiểm tra
    ...
    public boolean isAllowedItem(Material material) {
        ...
        // Sử dụng phương pháp whitelist - vật phẩm CHỈ được phép nếu thuộc một trong
        // các loại được cho phép
        return isOre(material) || isCrop(material) || isMobDrop(material);
    }
    ...
    public boolean isCustomItemWhitelisted(String id, String type) {
        ...
        if ("mmoitems".equalsIgnoreCase(type)) {
            return mmoItemsWhitelist.contains(id) || mmoItemsWhitelist.contains(fullId)
                    || mmoItemsWhitelist.contains("*");
        } else {
            return customItemsWhitelist.contains(id) || customItemsWhitelist.contains(fullId)
                    || customItemsWhitelist.contains("*");
        }
    }
}
