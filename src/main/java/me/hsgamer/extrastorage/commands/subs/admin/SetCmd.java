...
@Command(value = "set", usage = "/{label} set <material-key> <amount> [player]", permission = Constants.ADMIN_SET_PERMISSION, minArgs = 2)
public final class SetCmd extends CommandListener {
    ...
    public void execute(CommandContext context) {
        ...
        if (context.getArgsLength() >= 3) {
            ...
            if (space != -1) {
                long usedSpace = storage.getUsedSpace() - optional.get().getQuantity();
                if ((usedSpace + amount) > space) {
                    amount = (space - usedSpace);
                }
            }
            ...
            if (player.isOnline()) {
                player.getPlayer().sendMessage(Message.getMessage("SUCCESS.Set.target")
                        .replaceAll(QUANTITY_REGEX, Digital.formatThousands(amount))
                        .replaceAll(ITEM_REGEX, setting.getNameFormatted(args0, true))
