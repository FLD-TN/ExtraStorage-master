...
public final class WithdrawCmd
    ...
    public void execute(CommandContext context) {
        ...
        if (context.getArgsLength() == 0) {
            context.sendMessage(Message.getMessage("FAIL.missing-args").replace("{usage}",
                    "/" + context.getLabel() + " withdraw <material-key> [amount]"));
            return;
        }
        ...
        if (!optional.isPresent()) {
            context.sendMessage(Message.getMessage("FAIL.item-not-in-storage").replaceAll(Utils.getRegex("player"),
                    player.getName()));
            return;
        }
        ...
        if (current < 1) {
            context.sendMessage(Message.getMessage("FAIL.not-enough-item").replaceAll(Utils.getRegex("item"),
