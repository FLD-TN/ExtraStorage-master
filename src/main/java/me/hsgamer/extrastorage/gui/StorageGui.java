...
public final class StorageGui
    ...
    public StorageGui(Player player, User partner, int page) {
        ...
        this.handleClick(event -> {
            // Chỉ hủy sự kiện nếu người chơi click vào inventory của mình
            if (!event.isTopClick()) {
                event.setCancelled(true);
            }
        });
    ...
    private void load() {
        ...
        switch (sort) {
            ...
            case UNFILTER:
                ...
                break;
            case TIME:
                // Mặc định sử dụng sắp xếp theo vật liệu nếu là TIME
                this.addSortByMaterial();
                break;
        }
    ...
    private void addRepresentItem() {
        ...
        for (startIndex = (page - 1) * slots.length; startIndex < endIndex; startIndex++) {
            ...
            if (!name.isEmpty())
                ...
                meta.setDisplayName(setting.getNameFormatted(key, true));
                ...
                for (int i = 0; i < newLore.size(); i++) {
                    String lore = newLore.get(i)
                            .replaceAll(Utils.getRegex("status"),
                                    Message.getMessage("STATUS." + (item.isFiltered() ? "filtered" : "unfiltered")))
            ...
            Icon icon = new Icon(iStack)
                    .handleClick(event -> {
                        ...
                        if (click == ClickType.SHIFT_RIGHT) {
                            ...
                            for (ItemStack is : items) {
                                ...
                                if (item.getType() != i.getType())
                                    continue;
                                if (!item.getKey().equalsIgnoreCase(ItemUtil.toMaterialKey(is)))
                                    continue;
                            ...
                            if (count == 0) {
                                player.sendMessage(Message.getMessage("FAIL.not-enough-item-in-inventory")
                                        .replaceAll(Utils.getRegex("item"), setting.getNameFormatted(key, true)));
                                return;
                            }

                            if (instance.getSetting().isLogTransfer()) {
                                instance.getLog().log(player, partner.getOfflinePlayer(), Log.Action.TRANSFER, key,
                                        count, -1);
                            }
                        ...
                        if (current <= 0) {
                            player.sendMessage(Message.getMessage("FAIL.not-enough-item")
                                    .replaceAll(Utils.getRegex("item"), setting.getNameFormatted(key, true)));
                            return;
                        }
                        ...
                        if (click == ClickType.SHIFT_LEFT)
                            ...
                            ; // Bỏ qua vì phần này chỉ rút 1 vật phẩm.
                        ...
                        if (instance.getSetting().isLogWithdraw()) {
                            instance.getLog().log(player, partner.getOfflinePlayer(), Log.Action.WITHDRAW, key, free,
                                    -1);
                        }
    ...
    private void addAboutItem() {
        ...
        Icon icon = new Icon(
                ...
                .handleClick(event -> {
                    boolean isAdminOrSelf = (this.hasPermission(Constants.ADMIN_OPEN_PERMISSION)
                            || partner.getUUID().equals(player.getUniqueId()));
                    if ((!this.hasPermission(Constants.PLAYER_TOGGLE_PERMISSION)) || (!isAdminOrSelf))
                        return;
                ...
                .handleClick(event -> {
                    ...
                    SortType newSort = (event.isShiftClick() ? sort
                            : (event.isLeftClick() ? SortType.NAME
                                    : (event.isRightClick() ? SortType.UNFILTER : null)));
                    if (newSort == null)
                        return;
    ...
    private void addSortByQuantity() {
        ...
        Icon icon = new Icon(
                ...
                .handleClick(event -> {
                    ...
                    SortType newSort = (event.isShiftClick() ? sort
                            : (event.isLeftClick() ? SortType.UNFILTER
        ...
        Icon icon = new Icon(
                ...
                .handleClick(event -> {
                    ...
                    if (event.isLeftClick()) {
                        // Chuyển GUI theo chiều ngược (Storage → Sell → Partner → Filter)
                        ...
                        // Chuyển GUI theo chiều thuận (Storage → Filter → Partner → Sell)
    ...
    private int getFreeSpace(ItemStack item) {
        ...
        if (empty > 0)
            return Math.min(empty, item.getAmount());
        return -1;
    }
