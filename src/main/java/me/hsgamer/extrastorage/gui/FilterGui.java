...
public final class FilterGui
    ...
    private void load() {
        ...
        this.addFilterManagerButton();
        this.addNewItemButton(); // Thêm nút mới để thêm block
        ...
        switch (sort) {
            ...
            case QUANTITY:
                ...
                break;
            case TIME:
                // Mặc định sắp xếp theo vật liệu
                this.addSortByMaterial();
                break;
            case UNFILTER:
                // Nếu có
    ...
    /**
     * Thêm nút để mở giao diện thêm vật phẩm mới vào bộ lọc
     */
    private void addNewItemButton() {
        int[] slots = this.getSlots("ControlItems.NewItemButton");
        if ((slots == null) || (slots.length < 1))
            return;

        final String PATH = "ControlItems.NewItemButton";
        ...
        Icon icon = new Icon(
                this.getItemStack(
                        ...
                        user,
                        meta -> {
                            String name = config.getString(PATH + ".Name", "§aThêm Block Mới");
                            if (!name.isEmpty())
                            ...
                            if (lores.isEmpty()) {
                                lores = Arrays.asList(
                                        "§7Thêm block mới vào bộ lọc",
                                        "§7ngay cả khi chưa có trong kho");
                            }
                            meta.setLore(lores);
                ...
                .handleClick(event -> {
                    ...
                    // Kiểm tra xem bộ lọc có bị khóa không
                    if (!ExtraStorage.isFilterEnabled() || !storage.getStatus()) {
                        // Thông báo cho người chơi biết bộ lọc đang bị khóa
                        player.sendMessage(Message.getMessage("FAIL.invalid-filter"));
                        return;
                    }

                    // Mở giao diện thêm vật phẩm
                    try {
                        new AddItemToStorageGui(player).open();
                    } catch (Exception e) {
                        // Nếu lỗp AddItemToStorageGui không tồn tại, thông báo cho người chơi
                        player.sendMessage(Message.getMessage("FAIL.filter-error"));
                        e.printStackTrace();
                    }
                }).setSlots(slots);
    ...
    private void addRepresentItem() {
        ...
        for (startIndex = (page - 1) * slots.length; startIndex < endIndex; startIndex++) {
            ...
            if (!name.isEmpty())
                ...
                meta.setDisplayName(instance.getSetting().getNameFormatted(key, true));

            List<String> curLore = (meta.hasLore() ? meta.getLore() : new ArrayList<>()),
                    newLore = config.getStringList("RepresentItem.Lore");
            if (!newLore.isEmpty()) {
                for (int i = 0; i < newLore.size(); i++) {
                    String lore = newLore.get(i).replaceAll(Utils.getRegex("quantity"),
                            Digital.formatThousands(item.getQuantity()));
                    newLore.set(i, lore);
                }
            ...
            Icon icon = new Icon(iStack)
                    .handleClick(event -> {
                        ...
                        // Mở menu xác nhận thay vì xóa trực tiếp
                        new ConfirmUnfilterGui(player, key).open();
                    })
    ...
    private void addAboutItem() {
        ...
        Icon icon = new Icon(
                this.getItemStack(
                        ...
                        meta -> {
                            ...
                            if (!lores.isEmpty()) {
                                ...
                                long space = storage.getSpace(), used = storage.getUsedSpace(),
                                        free = storage.getFreeSpace();
                                double usedPercent = storage.getSpaceAsPercent(true),
                                        freePercent = storage.getSpaceAsPercent(false);
                                ...
                                for (int i = 0; i < lores.size(); i++) {
                                    String lore = lores.get(i)
                                            ...
                                            .replaceAll(Utils.getRegex("status"),
                                                    Message.getMessage(
                                                            "STATUS." + (storage.getStatus() ? "enabled" : "disabled")))
                                            .replaceAll(Utils.getRegex("space"),
                                                    (space == -1) ? UNKNOWN : Digital.formatThousands(space))
                                            .replaceAll(Utils.getRegex("used(\\_|\\-)space"),
                                                    (used == -1) ? UNKNOWN : Digital.formatThousands(used))
                                            .replaceAll(Utils.getRegex("free(\\_|\\-)space"),
                                                    (free == -1) ? UNKNOWN : Digital.formatThousands(free))
                                            .replaceAll(Utils.getRegex("used(\\_|\\-)percent"),
                                                    (usedPercent == -1) ? UNKNOWN : (usedPercent + "%"))
                                            .replaceAll(Utils.getRegex("free(\\_|\\-)percent"),
                                                    (freePercent == -1) ? UNKNOWN : (freePercent + "%"));
                                    lores.set(i, lore);
                                }
                                meta.setLore(lores);
                            }
                ...
                .handleClick(event -> {
                    if (items.isEmpty() || (!event.isShiftClick()))
                        return;
    ...
    private void addSortByMaterial() {
        ...
        Icon icon = new Icon(
                ...
                .handleClick(event -> {
                    SortType newSort = (event.isShiftClick() ? sort
                            : (event.isLeftClick() ? SortType.NAME
                                    : (event.isRightClick() ? SortType.QUANTITY : null)));
                    if (newSort == null)
                        return;
    ...
    private void addSortByName() {
        ...
        Icon icon = new Icon(
                ...
                .handleClick(event -> {
                    SortType newSort = (event.isShiftClick() ? sort
                            : (event.isLeftClick() ? SortType.QUANTITY
                                    : (event.isRightClick() ? SortType.MATERIAL : null)));
                    if (newSort == null)
                        return;
    ...
    private void addSortByQuantity() {
        ...
        Icon icon = new Icon(
                ...
                .handleClick(event -> {
                    SortType newSort = (event.isShiftClick() ? sort
                            : (event.isLeftClick() ? SortType.MATERIAL
                                    : (event.isRightClick() ? SortType.NAME : null)));
                    if (newSort == null)
                        return;
        ...
        Icon icon = new Icon(
                ...
                .handleClick(event -> {
                    ...
                    if (event.isLeftClick()) {
                        // Chuyển GUI theo chiều ngược (Filter → Storage → Sell → Partner)
                        if (this.hasPermission(Constants.PLAYER_OPEN_PERMISSION))
                            new StorageGui(player, 1).open();
                        else if (this.hasPermission(Constants.PLAYER_SELL_PERMISSION))
                            new SellGui(player, 1).open();
                        else if (this.hasPermission(Constants.PLAYER_PARTNER_PERMISSION))
                            new PartnerGui(player, 1).open();
                    } else if (event.isRightClick()) {
                        // Chuyển GUI theo chiều thuận (Filter → Partner → Sell → Storage)
                        if (this.hasPermission(Constants.PLAYER_PARTNER_PERMISSION))
                            new PartnerGui(player, 1).open();
                        else if (this.hasPermission(Constants.PLAYER_SELL_PERMISSION))
                            new SellGui(player, 1).open();
    ...
    private void addFilterManagerButton() {
        int[] slots = this.getSlots("ControlItems.FilterManager");
        if ((slots == null) || (slots.length < 1))
            return;

        final String PATH = "ControlItems.FilterManager";
        ...
        Icon icon = new Icon(
                ...
                .handleClick(event -> {
                    ...
                    new FilterManagerGui(player, 1).open();
                }).setSlots(slots);
